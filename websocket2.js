...
class Fish {
  ...
  _initParts(roomId) {
    ...
    // FIX: Each segment needs its own rot matching its actual direction
    // so the chain is compact and the body looks natural from frame 1.
    // Place each segment directly behind the previous one using that
    // segment's rot (= head.rot initially, so chain is straight).
    // Give each segment a tiny angular velocity so the chain relaxes
    // into a natural curve immediately rather than staying rigid.
    let prevX = head.x, prevY = head.y, prevRot = head.rot;
    for (let i = 1; i <= this.breakPoint; i++) {
      const seg    = new Part();
      const behind = prevRot + Math.PI;
      seg.x   = prevX + Math.cos(behind) * this.size;
      seg.y   = prevY + Math.sin(behind) * this.size;
      // Each segment inherits the direction of its predecessor
      seg.rot = prevRot;
      // Small random vt so segments immediately start to wiggle naturally
      // instead of locking into a perfectly rigid stick
      seg.vt  = (Math.random() - 0.5) * 0.5;
      this.parts.push(seg);
      prevX = seg.x; prevY = seg.y; prevRot = seg.rot;
    }
  }
  ...
  update(dt, roomConfig) {
    ...
    // Body chain
    // FIX: Each segment is positioned behind PREV using PREV's rotation,
    // not its own. Using self.rot caused the chain to drift and look
    // stretched because segments could face any direction.
    // The break segment (tail) now has a spring keeping it behind seg[bp-1].
    let prev = null;
    for (let i = 0; i < this.parts.length; i++) {
      ...
      if (i === this.breakPoint) {
        // Break segment: physics-simulated, but spring-anchored to the
        // tail of the normal chain so it never floats away visually.
        p.updatePosRot(dt);
        p.dampVel(0.8, dt);
        p.dampRot(0.05, dt);
        if (prev) {
          const behind  = prev.rot + Math.PI;
          const targetX = prev.x + Math.cos(behind) * this.size;
          const targetY = prev.y + Math.sin(behind) * this.size;
          p.vx += (targetX - p.x) * 40 * dt;
          p.vy += (targetY - p.y) * 40 * dt;
          p.vt += normalizeAngle(prev.rot - p.rot) * 10 * dt;
        }
        prev = p;
      } else if (prev) {
        ...
        // FIX: place behind PREV using PREV's rotation (not self.rot)
        const behind = prev.rot + Math.PI;
...
class GameRoom {
  ...
  respawnPlayer(player) {
    ...
    // FIX: same compact chain init as _initParts — each segment behind prev
    let prevX=head.x, prevY=head.y, prevRot=head.rot;
    for(let i=1;i<=player.breakPoint;i++){
      const seg=new Part(), behind=prevRot+Math.PI;
      seg.x=prevX+Math.cos(behind)*player.size;
      seg.y=prevY+Math.sin(behind)*player.size;
      seg.rot=prevRot;
      seg.vt=(Math.random()-0.5)*0.5;
      player.parts.push(seg);
      prevX=seg.x; prevY=seg.y; prevRot=seg.rot;
    }
  }
